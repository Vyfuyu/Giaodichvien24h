...
function formatProfile(
  ...
  return {
    ...
    userAvatar: profile.avatar ?? user?.avatar ?? null,
...
router.get("/middlemen", async (_req, res): Promise<void> => {
  ...
  res.json(profiles.map(({ middleman_profiles, users }) =>
    formatProfile(middleman_profiles, users ? { name: users.name, avatar: users.avatar } : null)
  ));
});

router.get("/middlemen/:id", async (req, res): Promise<void> => {
  ...
  const [row] = await db
    ...
    .from(middlemanProfilesTable)
    .leftJoin(usersTable, eq(middlemanProfilesTable.userId, usersTable.id))
    .where(eq(middlemanProfilesTable.id, id));
  ...
  res.json(formatProfile(row.middleman_profiles, row.users ? { name: row.users.name, avatar: row.users.avatar } : null));
});
